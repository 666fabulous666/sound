...
impl Scheduler {
        ...
        while self.sched_start < now + SCHEDULER_WAKE_EARLY {
            ...
            // ---- generate notes from sequences that need it ----
            {
                for seq in self.sequences.clone().iter() {
                    ...
                    {
                        self.draw_seq_at(seq.token, rng, &mut notes_buffer);
                    }
                }
            }
    ...
    fn draw_seq_at(&mut self, a: usize, rng: &mut ThreadRng, out: &mut Vec<(usize, Vec<Note>)>) {
    ...
    fn regen_seq_at(
        ...
        self.remove_seq(a);
        self.draw_seq_at(a, rng, notes_buffer);
